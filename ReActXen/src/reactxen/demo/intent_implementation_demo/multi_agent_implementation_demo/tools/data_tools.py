...
def _extract_fd_number(dataset_name: str) -> str:
    ...
    # Extract FD number - look for FD followed by digits
    match = re.search(r"FD(\d+)", normalized)
    if match:
        return f"FD{match.group(1)}"

    # Fallback: if it's just the FD number
    if normalized.startswith("FD") and normalized[2:].isdigit():
        return normalized

    # If no FD found, return as-is (might be just a number)
...
class LoadDatasetTool(BaseTool):
    ...
    def _run(self, dataset_name: str, split: str = "train") -> str:
        ...
        # Strip quotes from parameters (handles cases where LLM includes quotes)
        ...
        if "CMAPSS" in normalized_name:
            # Extract FD number (e.g., "FD001" from "CMAPSS_FD001")
            fd_num = _extract_fd_number(normalized_name)

            # Construct file path: train_FD001.txt or test_FD001.txt
            if split == "train":
            ...
            if file_path.exists():
                try:
                    # Read space-separated text file (CMAPSS format)
...
class LoadGroundTruthInput(BaseModel):
    ...
    file: str = Field(
        default="",  # Make optional with empty default
        description="Ground truth file name. For CMAPSS_FD001, use 'RUL_FD001.txt'. If empty, will auto-detect from dataset name.",
    )
...
class LoadGroundTruthTool(BaseTool):
    ...
    def _run(self, dataset_name: str, file: str = "") -> str:
        # Strip quotes
        dataset_name = _strip_quotes(dataset_name)
        if file:
        ...
        # For CMAPSS, auto-detect file name if not provided or if file doesn't exist
        if "CMAPSS" in normalized_name:
            fd_num = _extract_fd_number(normalized_name)

            # If file not provided or doesn't exist, try RUL_FD001.txt
...
class LoadGroundTruthInput(BaseModel):
    ...
    file: str = Field(
        default="",  # ensures tool is optional
