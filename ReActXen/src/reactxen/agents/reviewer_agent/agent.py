from reactxen.agents.reviewer_agent.reviewer_prompt import system_prompt_template
from reactxen.utils.model_inference import watsonx_llm
import json
import re

REVIEW_HEADER = "We evaluated the effectiveness and accuracy of an AI agent's response to a given question or task. The task did not complete successfully. The following feedback provides a plan to avoid failing in the same way again. Use them to improve your strategy.\n"


class ReviewerAgent:
    """
    A class to encapsulate the logic for the ReviewerAgent, which evaluates the success or failure
    of an AI agent's response based on given criteria.
    """

    def __init__(
        self,
        review_prompt=system_prompt_template,
        llm=watsonx_llm,
        model_id=6,
        max_retries=3,
    ):
