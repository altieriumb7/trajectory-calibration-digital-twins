"""PHMForge benchmark runner with integrated telemetry logging.
"""
...
os.environ["PHMFORGE_DATA_DIR"] = str(_PROJECT_ROOT / "data" / "CMAPSSData")
...
# Patch watsonx_llm to handle generate() exceptions
...
_extra_ctx = {
    ...
    "gemini/gemini-1.5-flash": 128000,
}
...
MODEL_NAME_TO_ID = {
    ...
    "mistralai/mistral-medium-2505": 20,
    "mistralai/mistral-small-3-1-24b-instruct-2503": 21,
    "openai/gpt-oss-120b": 22,
    "openai/gpt-4o-mini": 23,
    "gemini/gemini-1.5-flash": 24,
}

FRAMEWORK_CONFIGS = {
    "react": {"num_reflect_iteration": 1, "max_steps": 8, "max_execution_time": 480},
    "reactxen": {"num_reflect_iteration": 3, "max_steps": 8, "max_execution_time": 480},
}
...
def run_one_scenario(scenario: dict, framework: str, model_id: int) -> dict:
    ...
    try:
        agent = create_reactxen_agent(
            ...
            log_structured_messages=True,  # Crucial for telemetry capturing!
            debug=False,
        )
        agent.run()
    ...
    # Telemetry logging to results/telemetry_runs
    if agent:
        try:
            ...
            logger.log_trajectory(
                ...
                scenario_id=record["task_id"],
                steps_log=getattr(agent, "json_log", []),
                final_answer=record["answer"],
                task_success=record["correct"],
                raw_scratchpad=getattr(agent, "scratchpad", "")
            )
        except Exception as logger_err:
            print(f"    Warning: TelemetryLogger failed: {logger_err}")
